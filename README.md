...
**ProtoSwitch v0.1.0-beta.6** - CLI/TUI на Rust для Telegram Desktop с фокусом на Windows, который помогает быстро заменить MTProto proxy, если текущий перестал работать.

Проект рассчитан на пользователей Windows 10/11, которым нужен понятный инструмент без ручного копирования адресов, портов и `secret` в настройки Telegram. ProtoSwitch следит за состоянием текущего proxy, может подобрать новый с `mtproto.ru`, переждать их выбор сервера, применить proxy через официальный `tg://proxy?...` deep link и подтвердить подключение в Telegram без ручного клика, если окно подтверждения доступно в текущей пользовательской сессии.
...
## Что входит в релиз v0.1.0-beta.6
...
Первый публичный Windows-дистрибутив распространяется через GitHub Releases. Обновления в этой очереди устанавливаются вручную: автообновления в `v0.1.0-beta.6` еще не предусмотрены.
...
4. Завершите установку. Installer сам выполняет `protoswitch init --non-interactive --no-autostart` сразу после копирования файлов.
5. Если чекбокс автозапуска оставлен включенным, installer отдельно вызывает `protoswitch autostart install` для текущего пользователя.
6. Ярлык `ProtoSwitch` и прямой запуск `protoswitch.exe` теперь открывают интерфейс приложения: при первом старте это экран настройки, после инициализации это экран статуса.
7. После установки проверьте состояние через `protoswitch doctor` или `protoswitch status`.
...
ProtoSwitch в `v0.1.0-beta.6` обновляется вручную через страницу релизов GitHub.
...
6. Если Telegram Desktop уже запущен, ProtoSwitch передает в систему `tg://proxy?...`, затем ищет модальное окно Telegram и подтверждает подключение через Windows UI Automation.
...
3. В fallback-режиме используется Startup folder текущего профиля Windows, чтобы watcher все равно стартовал после входа в систему.
...
Такая схема нужна для систем, где Task Scheduler ограничен локальной политикой, правами пользователя или настройками безопасности, но обычный Startup folder все еще доступен.
...
- если `mtproto.ru` пишет, что выбор сервера ещё идёт, программа повторяет запросы серией попыток вместо мгновенного отказа;
- ищет в ней встроенную `tg://proxy` ссылку;
- извлекает из ссылки параметры подключения;
- если `mtproto.ru` отвечает, что свободных серверов нет, ProtoSwitch пишет эту причину явно в `status` и `doctor`;
...
`v0.1.0-beta.6` - ранняя Windows-дистрибуция. Основной фокус здесь на рабочем Windows-сценарии, понятной диагностике, удобной смене MTProto proxy и нормальной пользовательской поставке без необходимости запускать проект через `cargo`.
