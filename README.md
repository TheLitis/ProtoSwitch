...
**ProtoSwitch v0.1.0-beta.2** - CLI/TUI на Rust для Telegram Desktop с фокусом на Windows, который помогает быстро заменить MTProto proxy, если текущий перестал работать.
...
## Что умеет версия v0.1.0-beta.2
...
- Установка и удаление автозапуска с приоритетом Scheduled Task и автоматическим fallback в папку Startup текущего пользователя, если Windows отказывает в создании задачи.
- Показ фактического способа автозапуска в `status` и `doctor`, чтобы было видно, включен ли `scheduled_task` или `startup_folder`.
...
| `protoswitch autostart install` | Сначала пытается создать per-user Scheduled Task для автозапуска ProtoSwitch при входе в Windows, а при `Access is denied` / `Отказано в доступе` переключается на папку Startup текущего пользователя. |
...
2. Если Windows возвращает ошибку доступа вроде `Access is denied` или `Отказано в доступе`, ProtoSwitch автоматически переключается на папку автозапуска пользователя.
...
4. После установки автозапуска проверьте `protoswitch status` или `protoswitch doctor`, чтобы увидеть, включился ли `scheduled_task` или был использован `startup_folder`.
5. Для постоянного мониторинга используйте `protoswitch watch`.
6. Если нужно срочно сменить proxy вручную, выполните `protoswitch switch`.
7. Когда нужно быстро понять текущее состояние, используйте `protoswitch status`.
...
`v0.1.0-beta.2` - ранняя бета-версия. Основной фокус здесь на рабочем Windows-сценарии, понятной диагностике и удобной смене MTProto proxy без ручного редактирования Telegram.
