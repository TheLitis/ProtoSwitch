...
**ProtoSwitch v0.1.0-beta.8** - CLI/TUI на Rust для Telegram Desktop с фокусом на Windows, который помогает быстро заменить MTProto proxy, если текущий перестал работать.
...
## Что нового в v0.1.0-beta.8

- Интерфейс стал спокойнее и ближе к современному terminal-first console flow: меньше визуального шума, полезнее action list, отдельная команда cleanup.
- ProtoSwitch теперь валидирует кандидатов от `mtproto.ru` TCP-проверкой до применения и пишет явный статус источника и proxy в `status` и TUI.
- После применения нового proxy приложение старается вернуть фокус в предыдущее окно и не оставлять Telegram поверх всего рабочего стола.
- Мёртвые proxy из управляемого списка Telegram теперь можно удалять автоматически через cleanup flow, а installer умеет предлагать ярлык на рабочем столе.

## Что входит в релиз v0.1.0-beta.8
...
Первый публичный Windows-дистрибутив распространяется через GitHub Releases. Обновления в этой очереди устанавливаются вручную: автообновления в `v0.1.0-beta.8` еще не предусмотрены.
...
ProtoSwitch в `v0.1.0-beta.8` обновляется вручную через страницу релизов GitHub.
...
`v0.1.0-beta.8` - ранняя Windows-дистрибуция. Основной фокус здесь на рабочем Windows-сценарии, понятной диагностике, более спокойном terminal-first интерфейсе и нормальной пользовательской поставке без необходимости запускать проект через `cargo`.
