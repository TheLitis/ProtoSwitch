...
**ProtoSwitch v0.1.0-beta.10** - CLI/TUI на Rust для Telegram Desktop с фокусом на Windows, который помогает быстро заменить MTProto proxy, если текущий перестал работать.
...
## Что нового в v0.1.0-beta.10

- Перед добавлением proxy в Telegram ProtoSwitch теперь старается вызвать `Check Status` в proxy-диалоге и не засоряет список первым попавшимся кандидатом, если Telegram сразу показывает отказ.
- Применение proxy теперь считается успешным только после финального статуса `Available` внутри Telegram: `Checking`, `Unknown` и `Missing` больше не маскируются под рабочее подключение.
- Если кандидат не подтвердился, `watch` и `switch` автоматически переходят к следующему адресу из provider pool, а не зависают на одном неудачном proxy.
- Подтверждение `Add/Connect` в Telegram по-прежнему выполняется автоматически без ручного клика пользователя, но теперь после более жёсткой предвалидации.

## Что входит в релиз v0.1.0-beta.10
...
Первый публичный Windows-дистрибутив распространяется через GitHub Releases. Обновления в этой очереди устанавливаются вручную: автообновления в `v0.1.0-beta.10` еще не предусмотрены.
...
ProtoSwitch в `v0.1.0-beta.10` обновляется вручную через страницу релизов GitHub.
...
- валидирует кандидатов локальным TCP/SOCKS5 health-check и дополнительной проверкой через proxy-диалог Telegram перед подтверждением;
...
`v0.1.0-beta.10` - ранняя Windows-дистрибуция. Основной фокус здесь на рабочем Windows-сценарии, понятной диагностике, более сильном terminal-first интерфейсе и нормальной пользовательской поставке без необходимости запускать проект через `cargo`.
