...
**ProtoSwitch v0.1.0-beta.5** - CLI/TUI на Rust для Telegram Desktop с фокусом на Windows, который помогает быстро заменить MTProto proxy, если текущий перестал работать.
...
## Что входит в релиз v0.1.0-beta.5
...
Первый публичный Windows-дистрибутив распространяется через GitHub Releases. Обновления в этой очереди устанавливаются вручную: автообновления в `v0.1.0-beta.5` еще не предусмотрены.
...
ProtoSwitch в `v0.1.0-beta.5` обновляется вручную через страницу релизов GitHub.
...
`v0.1.0-beta.5` - ранняя Windows-дистрибуция. Основной фокус здесь на рабочем Windows-сценарии, понятной диагностике, удобной смене MTProto proxy и нормальной пользовательской поставке без необходимости запускать проект через `cargo`.
